...
def process_args(default_concur_req):
    ...
    # judge if these are right arguments
    if args.max_req < 1:
        print('*** Usage error: --max_req CONCURRENT must be >= 1')
        parser.print_usage()   # print right usage
        sys.exit(1)   # Stop the routine
    if args.limit < 1:
    ...
    try:
        ...
        print(exc.args[0])   # capture the value of exception
        parser.print_usage()
        sys.exit(1)
    if not cc_list:
...
def main(download_many, default_concur_req, max_concur_req):
    ...
    assert sum(counter.values()) == len(cc_list), 'some downloads are unaccounted for'   # print message if fails
